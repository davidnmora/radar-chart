...
class RadarChart {
	constructor(props) {
		...
		this.id = id
		this.radialUtils = new RadialUtilsManager(data, options)
	}
	
	render() {
		// (hack so I can keep my es6 arrow fns)
		const radialUtils = this.radialUtils
		...
		axis.append("text")
			...
			.text(d => d)
			.call(radialUtils.wrap, radialUtils.config.wrapWidth);
